    ...
    try {
        if (!forms.value.id) {
            await http.post("/api/warehouse/output-invoice/", {
                ...
                if (res.status === 201) {
                    router.push({name: "output-invoice_item", params: {id: res.data.id}})
                }
            })
        } else {
            await http.put(`/api/warehouse/output-invoice/${forms.value.id}/`, {
                description: forms.value.description
            }).then(res => {
                if (res.status === 200) {
                    location.reload()
...
async function deleteModal() {
    try {
        await http.delete(`/api/warehouse/output-invoice/${forms.value.id}/`).then(res => {
            if (res.status === 204) {
                location.reload()
