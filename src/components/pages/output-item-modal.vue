<template>
    <appModal v-model="dialog">
        <div class="modal1">
            <h3 v-if="!forms.id">Add output invoice</h3>
            <h3 v-else>Edit output invoice</h3>
            <Form @submit="sendInvoice">
                ...
                <button type="submit" v-if="!forms.id">Add output invoice</button>
                <button type="submit" v-else>Edit output invoice</button>
            </Form>
        </div>
    </appModal>
...
async function sendInvoice() {
    try {
        if (!forms.value.id) {
            await http.post("/api/warehouse/output-invoice-item/", {
            ...
            await http.put(`/api/warehouse/output-invoice-item/${forms.value.id}/`, {
                quantity: forms.value.quantity,
                price: forms.value.price
            }).then(res => {
                if (res.status === 200) {
...
async function deleteModal() {
    try {
        await http.delete(`/api/warehouse/output-invoice-item/${forms.value.id}/`).then(res => {
            if (res.status === 204) {
                location.reload()
