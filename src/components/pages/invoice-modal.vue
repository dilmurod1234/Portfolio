    ...
    <appModal v-model="dialog">
        <div class="modal1">
            ...
            <Form @submit="sendInvoice">
                <label v-if="!forms.id" for="supplier">Select supplier</label>
                <Field v-if="!forms.id" rules="required" :modelValue="forms.supplier" v-slot="{ errors }" name="supplier">
                    <select v-model="forms.supplier">
    ...
    try {
        if (!forms.value.id) {
            await http.post("/api/warehouse/input-invoice/", {
                ...
                if (res.status === 201) {
                    router.push({name: "input-invoice_item", params: {id: res.data.id}})
                }
            })
        } else {
            await http.put(`/api/warehouse/input-invoice/${forms.value.id}/`, {
                description: forms.value.description
            }).then(res => {
                if (res.status === 200) {
                    location.reload()
...
async function deleteModal() {
    try {
        await http.delete(`/api/warehouse/input-invoice/${forms.value.id}/`).then(res => {
            if (res.status === 204) {
                location.reload()
