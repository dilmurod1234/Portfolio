<template>
    <section>
        <invoiceOutputModal ref="output_modal"/>
        <div class="invoice__item-bar">
            <div class="invoice__item">
                <span class="invoice__item-text">{{ output_invoice?.client?.title }}</span>
                <span class="invoice__item-text">{{ output_invoice?.total_sum }}</span>
            </div>
            <button v-if="role !== 'director' && output_invoice?.status !== 'confirmed'" class="product__btn" @click="openModal">Add product</button>
        </div>
        <appTable :headers="headers" :body="output_invoice_list">
            ...
            <template #body_action="{ item }">
                <span class="edit" v-if="output_invoice?.status !== 'confirmed' && role !== 'director'" @click="openModalEdit(item)"><i class="fa-solid fa-pen-to-square"></i></span>
                <span class="delete" v-if="output_invoice?.status !== 'confirmed' && role !== 'director'" @click="openModalDelete(item)"><i class="fa-solid fa-trash-can"></i></span>
            </template>
        </appTable>
        <div class="actions">
            <router-link class="btn" to="/output-invoice" v-if="output_invoice?.status !== 'confirmed'">Save</router-link>
            <button class="btn" v-if="output_invoice?.status !== 'confirmed'" @click="confirmInvoice">Confirm</button>
            <button class="btn" v-if="output_invoice?.status !== 'confirmed'" @click="deleteInvoice">Delete</button>
        </div>
    </section>
...
import invoiceOutputModal from "@/components/pages/output-item-modal.vue";
...
const output_invoice_list = ref([])
const output_invoice = ref({})
const output_modal = ref()
...
const getOutputInvoiceItem = () => {
    http.get(`/api/warehouse/output-invoice-item/`, {
        ...
        output_invoice_list.value = res.data.results
...
async function confirmInvoice() {
    try {
        await http.post(`/api/warehouse/output-invoice/${url[4]}/confirm/`).then(res => {
            if (res.status === 200) {
                router.push({name: "output-invoice"})
...
const openModal = () => {
    output_modal.value.openModal()
}

const openModalEdit = (item) => {
    output_modal.value.openModal(item)
}

const openModalDelete = (item) => {
    output_modal.value.openDeleteModal(item)
}
