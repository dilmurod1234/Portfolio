<template>
    <section>
        <expenceModal ref="expence_modal"/>
        <div class="invoice__item-bar">
            <button class="hidden">0</button>
            <button v-if="role !== 'director'" class="product__btn" @click="openModal">Add expense</button>
        </div>
        <appTable :headers="headers" :body="finance_expence">
        </appTable>
    </section>
    <div class="pagination">
        <VPagination v-model="params_item.page" :pages="params_item.last_page" :range-size="1" active-color="#EAF5FF"
            @update:model-value="getExpence" />
    </div>
</template>
<script setup>
import { ref, onMounted } from "vue";
import expenceModal from "@/components/pages/expence-modal.vue";
import http from "@/plugins/axios";
import appTable from "@/components/ui/app-table.vue";
import VPagination from "@hennge/vue3-pagination"
import store from "@/store";

const finance_expence = ref([])
const expence_modal = ref()
...
const getExpence = () => {
    http.get(`/api/finance/expense/`, {
        ...
        finance_expence.value = res.data.results
