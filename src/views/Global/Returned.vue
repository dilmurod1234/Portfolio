<template>
    <section class="invoice">
        <returnedModal ref="returned"/>
        <button class="product__btn" @click="openModal" v-if="role !== 'director'">Add returned invoice</button>
        <appTable :headers="headers" :body="returned_invoice">
            ...
            <template #body_clients_phone="{ item }">
                <span>{{ item.client.phone_number }}</span>
            </template>
            <template #body_action="{ item }" #body_status="{item}">
                <router-link v-if="role !== 'director'" :to="{ name: 'returned-invoice_item', params: { id: item.id } }"
                    class="product__lists-icon">
                    <span><i class="fa-solid fa-eye"></i></span>
                </router-link>
                <router-link v-else :to="{ name: 'returned-invoice_item_d', params: { id: item.id } }"
                    class="product__lists-icon">
                    <span><i class="fa-solid fa-eye"></i></span>
                </router-link>
    ...
    <div class="pagination">
        <VPagination v-model="params.page" :pages="params.last_page" :range-size="1" active-color="#EAF5FF"
            @update:model-value="getreturnedInvoice" />
    </div>
</template>
<script setup>
...
const headers = ref([
    ...
    { title: "Phone number", value: "clients_phone" },
    { title: "Status", value: "status" },
    { title: "Description", value: "description" },
    { title: "Action", value: "action" }
])

const getreturnedInvoice = () => {
    http.get("/api/warehouse/returned-invoice", {
        ...
        returned_invoice.value = res.data.results
...
const openModal = () => {
    returned.value.openModal()
}

const openModalEdit = (item) => {
    returned.value.openModal(item)
}

const openModalDelete = (item) => {
    returned.value.openDeleteModal(item)
}
